...
if not model_accuracy:
    if test_model:
        ...
        model = Pipeline([
            ('preprocessor', preprocessor),
            ('rfe', RFE(estimator=XGBClassifier(learning_rate=0.05, max_depth=2, n_estimators=500, random_state=42), n_features_to_select=5))
        ])
    ...
    gt_file_path = "EvaluationGT.csv"
    gt_data = pd.read_csv(gt_file_path, header=None)
    y_true = gt_data.iloc[:, 0]
    ...
    print(f"True model accuracy is: {accuracy:.4f}")